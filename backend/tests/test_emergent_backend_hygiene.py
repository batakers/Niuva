    ...
    def test_active_backend_has_no_emergent_storage_integration(self):
        ...
        for path in active_backend_files:
            ...
            with self.subTest(path=str(path.relative_to(REPOSITORY_ROOT))):
                self.assertNotIn("integrations.emergentagent.com", source)
                self.assertNotIn("emergent_llm_key", source)
                self.assertNotIn("emergentintegrations", source)

    def test_active_deployment_guidance_has_no_emergent_runtime_reference(self):
