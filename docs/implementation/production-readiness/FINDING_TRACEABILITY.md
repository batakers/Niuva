...
| `auth_*`, password policy, identity routes, sessions, and their backend/frontend tests | Admin session, MFA, and access-review boundary; Recovery, password, bootstrap, and secret atomicity; Distributed abuse protection; Admin identity, permission, and selector parity | PRs #79–#81 add bounded Customer Session, Admin Session, and Password Recovery source/local-test evidence under `DEC-AUTH-003`, `DEC-AUTH-005`, and `DEC-AUTH-010`. Retain revalidation or blocked status for the grouped findings not exercised by those packets and for migration, production topology/delivery, MFA, access-review, and rollout gates. |
...
| Admin session, MFA, and access-review boundary | `FE-002`, `INT-004`, `SEC-002`, `SEC-003`, `SEC-007` | 01, 05, 06 | Legacy Admin bearer/session behavior and incomplete internal access controls conflict with approved auth direction. | P1 | Admin Session local revalidation merged in PR #80; MFA/access-review and production acceptance remain `requires_revalidation` / blocked | 94-100% | Yes | Yes | Decide the cross-tab policy; retain MFA, exact-target HTTPS/proxy, monitoring, migration/cutover, and rollout gates. | `DEC-AUTH-005/007`; [Feature 1.2 evidence](phases/FEATURE-1.2-admin-session-revalidation.md); identity and operations owners; Migration 009; browser and production evidence. |
| Recovery, password, bootstrap, and secret atomicity | `FE-003`, `BE-008`, `DB-002`, `SEC-004`, `SEC-005`, `SEC-013` | 01, 03, 04, 06 | Legacy recovery/password writes were not one approved, atomic, secret-safe contract. | P1 | Password Recovery bounded remediation evidence merged in PR #81; timing, provider delivery, password policy/bootstrap, and production gates remain open | 93-100% | Yes | Yes | Approve recovery timing evidence and production-delivery ownership; retain password-policy, bootstrap, migration, and rollout gates. | `DEC-AUTH-003/004`; ADR-001; [Feature 1.3 remediation](phases/FEATURE-1.3-password-recovery-remediation.md); Migration 008; isolated replica set; recovery runbook. |
