...
| Transaction guard and idempotency consistency | `BE-002` and `DB-003` are `resolved_in_source`; current-main Inquiry, Quote, Project Conversion, Work Order, and inactive Retail 3.4A are revalidated, while `INT-009` remains partial across command families outside that commerce audit |
| Retail payment and fulfilment capability enforcement | Current-main commerce revalidation confirms the legacy bypass is closed and Retail create/transition/payment mutations fail deterministically; `BE-003`/`INT-003` are resolved for the inactive scope and remain `blocked_by_decision` for any active transaction/payment/fulfilment journey |
...
| File ownership, content validation, and storage boundary | `BE-007`, `INT-010`, `SEC-008`, `SRE-007` | 03, 05, 06, 09 | Current-main revalidation confirms opaque-ID and logical-path reads are metadata/status/owner/domain scoped; client MIME is ignored; signature, bounded size/streaming, safe media, inactive/publication denial, and local metadata/object compensation pass. Historical payment-proof mutations are disabled and raw custody paths are withheld. | P1/P2 | `resolved_for_active_development_scope` / `blocked_by_decision_for_production` | 99% repository / 0% production storage | Yes for a file-enabled production release | Yes | Select provider/scanner, retention/quota/legal hold, RPO/RTO and owners; prove backup/restore, multi-instance consistency, orphan/historical-proof reconciliation, and compatibility migration before activation. | ADR-002; [current-main file/storage evidence](phases/FILE-STORAGE-CURRENT-MAIN-REVALIDATION-2026-08-14.md); DEC-PAY-02; object metadata; storage/operations owner. |
...
| Transaction guard and idempotency consistency | `BE-002`, `DB-003`, `INT-009` | 03, 04, 05 | Shared transaction adoption is merged. Current-main Inquiry/Quote conversion, Quote lifecycle, Project Conversion, Work Order, and inactive Retail 3.4A now have exact replay/conflict, version, fingerprint, or compare-and-swap evidence; command families outside the commerce audit are not globally closed. | P1 | `BE-002`/`DB-003` resolved_in_source; `INT-009` partial with current commerce revalidation | 97% for bounded repository scope / 0% production | Yes | Yes | Preserve exact-head real-transaction CI; reconcile remaining command families separately without inferring global retry safety or production topology. | ADR-001; shared executor; [current-main commerce evidence](phases/COMMERCE-LIFECYCLE-CURRENT-MAIN-REVALIDATION-2026-08-14.md); real replica-set failure tests. |
...
| Migration, live schema, backup, and restore safety | `DB-005`, `DB-006`, `DB-007`, `DB-008`, `DB-009`, `DB-010`, `DB-011`, `DB-012`, `DB-014`, `OPS-005`, `OPS-006`, `QA-004`, `GOV-013` | 04, 07, 08, 10 | Current-main Migration 001–010 review confirms uneven partial-failure, guard, rollback, marker/index ownership, and representative-data evidence. Migration 001/004/005/007 are not execution candidates; 006/008/009 remain controlled rehearsal candidates only; 010 is source-only. | P1/P2 | open / `partial` / `blocked_by_decision` / `environment_blocked`; repository revalidation complete, no data-bearing execution authorized | 96% repository / 0% production data | Yes for data-bearing release | Yes | Resolve source hard stops, then approve a distinct target, window, retention, RPO/RTO, restore, reviewer, and migration authority per candidate. | [current-main migration/data-integrity evidence](phases/MIGRATION-DATA-INTEGRITY-CURRENT-MAIN-REVALIDATION-2026-08-14.md); named target; backup owner; migration ledger; replica set; restore authority. |
...
| Retail payment and fulfilment capability enforcement | `BE-003`, `INT-003` | 03, 05 | The former legacy mutation bypass is closed. Retail 3.4A is a pure provider-neutral contract; create/transition return named `503`, legacy estimate/proof/verification return named `410`, and refund/return remain suspended. No active Retail transaction/payment/fulfilment journey exists. | P1 | `resolved_for_inactive_scope` / `blocked_by_decision_for_activation` | 98% repository / 0% production | No for provider-free inactive scope; yes for any promised active Retail scope | Yes | Keep all mutation capabilities inactive. Select provider/state/Finance/reconciliation/shipping and activation only through separate approved tasks. | ADR-003; DEC-PAY-02; [current-main commerce evidence](phases/COMMERCE-LIFECYCLE-CURRENT-MAIN-REVALIDATION-2026-08-14.md); provider-neutral capability boundary. |
...
| B2B organization/customer portal parity | `INT-007` | 05 | Current-main has permission-scoped internal Admin Inquiry/Quote/Project/Work Order routes and safe projection helpers, but no customer/organization route, membership model, assignment rule, or customer consumer journey. | P1 | `blocked_by_decision`; absence revalidated | 98% repository / 0% portal runtime | Yes for full B2B customer scope | Yes | Resolve DR-010 membership/assignment/ownership/history and route field contracts before any customer or organization query is mounted. | [current-main commerce evidence](phases/COMMERCE-LIFECYCLE-CURRENT-MAIN-REVALIDATION-2026-08-14.md); B2B release sequence; organization access decisions; seeded cross-organization test data. |
...
| API transport, validation, failure, and envelope | `FE-001`, `FE-005`, `FE-006`, `BE-001`, `INT-011` | 01, 03, 05 | Current-main has the compatible shared runtime envelope and bounded typed Auth/Order/B2B/Catalog contracts, but only 23/152 operations publish a success schema, 26 publish `ErrorEnvelope`, and none publishes OpenAPI security metadata. | P1/P2 | `partial`; current-main repository revalidation complete, whole-family schema/security/error and deployed-consumer evidence open | 98% repository / 0% external consumers | Yes | Yes | Add concrete models and separate Customer/Admin security metadata by bounded active family; align actual `4xx/409/410/503` outcomes without changing compatibility behavior. | [current-main API evidence](phases/API-CONTRACT-CURRENT-MAIN-REVALIDATION-2026-08-14.md); API governance; stable fixtures; selected RC and controlled release evidence. |
...
| Observability, background lifecycle, timeout, and capacity | `OPS-007`, `SRE-002`, `SRE-004`, `SRE-005`, `SRE-009` | 08, 09 | The [current-main runtime audit](phases/RUNTIME-RELIABILITY-CURRENT-MAIN-REVALIDATION-2026-08-14.md) revalidates local/source readiness truth, bounded worker lifecycle, lease/retry/exhaustion, redacted provider-neutral telemetry, resource bounds, index/pagination contracts, and a credential-free GET-only load probe. Owned service objectives, external telemetry/alerts, representative capacity, deployed topology, and controlled environment drills remain unproven. | P1/P2 | partial / `environment_blocked` | 92-98% | Yes for release candidate | Yes | Telemetry/SLO/on-call, production worker/deployment topology, capacity target, alert destination, and rollback owner. | External metrics/tracing and alert delivery; representative capacity model; staging-like dependency/load/failure environment. |
| QA and release gates | `QA-001`, `QA-002`, `QA-003`, `QA-005`, `QA-006`, `QA-007`, `OPS-002` | 07, 08 | Current-main required hermetic/transaction profiles are explicit and checksum-bearing; the public external smoke is manual and equally fail-closed. The collector pollution and empty-JUnit acceptance paths are corrected. External Admin evidence still permits absent artifacts and lacks a checksum manifest, while no approved external target was supplied. | P1/P2 | `resolved_in_repository_gate` for hermetic/transaction/public-smoke; `partial` / `environment_blocked` for external Admin, browser, parity, and production-like evidence | 98% repository / 0% external | Yes | Yes | Run exact-head required CI; approve external origins/roles and implement Admin artifact/checksum enforcement before a release-evidence claim; assign threshold/report retention ownership. | [current-main quality evidence](phases/QUALITY-EVIDENCE-CURRENT-MAIN-REVALIDATION-2026-08-14.md); seeded roles; approved HTTPS origins; supported browsers; isolated DB; release owner. |
| Package/runtime reproducibility | `OPS-001`, `GOV-001`, `GOV-002`, `GOV-005` | 08, 10 | Python 3.14.3 and the 72-distribution hash lock are consistently enforced by hermetic, transaction, and external-smoke workflows; the 30-entry direct input may use ranges because it is not the install artifact. | P1 | `resolved_in_repository_contract`; production artifact/support policy remains separate | 99% repository / 0% deployed artifact | Yes | Yes | Preserve reviewed lock regeneration and exact-head CI; assign production artifact/runtime support ownership separately. | [dependency reproducibility](phases/BACKEND-DEPENDENCY-REPRODUCIBILITY-2026-08-06.md); [current-main quality evidence](phases/QUALITY-EVIDENCE-CURRENT-MAIN-REVALIDATION-2026-08-14.md); release owner. |
| Dependency vulnerability and lifecycle risk | `SEC-012`, `GOV-003`, `GOV-004` | 06, 10 | Fresh backend audit reports zero known vulnerabilities and complete license metadata, but legal disposition remains open. `motor==3.3.1` is post normal-support EOL since 14 May 2026 and receives only critical fixes through 14 May 2027. | P1/P2 | vulnerability inventory current; license policy and Motor migration `blocked_by_decision` / `open` | 99% inventory / 0% lifecycle remediation | Yes until disposition | Yes | Assign legal review/exception expiry and authorize a separately tested Motor-to-PyMongo Async compatibility migration before final support ends. | Security/legal/release owners; [current-main quality evidence](phases/QUALITY-EVIDENCE-CURRENT-MAIN-REVALIDATION-2026-08-14.md); MongoDB migration guidance; transaction compatibility tests. |
| Governance, compatibility, and evidence provenance | `BE-011`, `OPS-011`, `OPS-012`, `GOV-008`, `GOV-009`, `GOV-011`, `GOV-012`, `GOV-014`, `GOV-015`, `GOV-016`, `GOV-017` | 03, 08, 10 | Current-main retains all 21 compatibility operations. Audit recommendation groups them as 14 retained, zero redirects, two deprecated, and five retirement-blocked tombstones; Contact write deprecation and read-only historical Contact retention are explicit, while runtime metadata/external evidence remain unchanged. | P1/P2 | `partial`; repository inventory current, external consumers/sunset/monitoring/release governance open | 98% repository / 0% external consumers | Conditional; `GOV-014` is release-blocking | Yes | Preserve every route until endpoint-specific consumer, communication, observation, successor, monitoring, and rollback gates are approved; correct stale Material sunset separately. | [current-main API evidence](phases/API-CONTRACT-CURRENT-MAIN-REVALIDATION-2026-08-14.md); compatibility register; decisions; runbooks; release record. |
| Maintainability, query, and frontend performance debt | `FE-007`, `FE-008`, `FE-009`, `BE-010`, `SRE-006`, `SRE-010`, `GOV-006`, `GOV-007`, `GOV-010` | 01, 03, 09, 10 | Merged Feature 8.2 bounds and stabilizes five Admin/B2B list queries; current-main audit confirms those contracts while Notification, Inventory, legacy Order, and other list continuation/date/query-plan behavior remains incomplete. Module/dependency debt and frontend performance/error budgets also remain open. | P2/P3 | `partial`; current-main repository revalidation complete for the five B2B families | 98% for repository contract / 0% representative load | Not alone | Conditional | Separately authorize indexes, query-plan/load evidence, raw/capped list families, budgets, and cleanup/refactor scope. | [current-main API evidence](phases/API-CONTRACT-CURRENT-MAIN-REVALIDATION-2026-08-14.md); performance targets; representative data model; owner approval. |
