...
| Subphase | Scope | Canonical findings | Blocking state and why | Exit criteria |
| --- | --- | --- | --- | --- |
| 8A - Truthful readiness and dependency health | Liveness/readiness routing, Mongo/dependency health, safe degradation contract. | Release artifact/readiness; notification/readiness | `source_authorized_environment_blocked`: `DEC-READY-01` approves the bounded dependency/probe contract; named routing/on-call ownership and staging-like evidence remain unavailable under `DR-012`/`DR-014`. | Approved probes never route traffic as ready when required dependencies are stale or failed. |
...
| 10C - Data recovery and rollback drills | Migration dry run, backup/restore, transaction and recovery failure, artifact rollback, and data rollback. | Migration/live-schema; transaction; recovery | Planning packet prepared on `ops/backend-recovery-drill`; execution remains `blocked_by_decision` and `blocked_by_environment` because PHASE-10A, target/owner/window/RPO-RTO/custody, immutable artifacts, accepted PHASE-02C evidence, and a staging-like replica set are absent. | Approved exact-SHA isolated drills prove restore and separate artifact/data rollback while preserving historical records. |
