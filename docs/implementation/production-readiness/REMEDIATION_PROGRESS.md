...
| 1 - Identity, Authentication, Authorization, and Privacy | Auth, role, privacy, file/order boundaries, transaction/payment guard | Session/MFA; recovery; password policy/hash migration; abuse; file/order; transaction; quote; payment; Admin parity | `requires_revalidation` — Feature 1.1–1.4 bounded source/local evidence merged through PRs #79–#81/#84; Feature 1.5–1.6 revalidation packets merged through PRs #85–#86; PHASE-01B selected-SHA evidence passed; independent review and broader decision/environment gates remain | `blocked_by_decision` | [Phase evidence index](phases/README.md); each packet retains its own boundary | PRs #79–#81/#84 for PHASE-01A; PRs #85–#86 inform PHASE-01D; PHASE-01B remains separate | `af625942cb64e3a4395f44bd57b74e5bacd7f7b1` current merged-evidence snapshot; selected RC remains `d04e3f0` | Customer/Admin Session, Password Recovery, and password-policy packets entered `main`; rate-limiter and MFA revalidations record current source/decision gaps. No migration, real provider delivery, production blocklist/Argon2 target evidence, production limiter topology, MFA implementation, deployment, or production acceptance was proven. | 2026-07-29 | Resolve Feature 1.1–1.4 residual gates and the `ADR-005`/`DEC-AUTH-004` password-rule clarification; complete DR-004/005 using Feature 1.5/1.6 gaps; obtain independent PHASE-01B review and preserve all migration/environment gates. |
...
| 3 - Backend Business Logic and API Contracts | HTTP envelope, lifecycle, readiness, notification, compatibility/CMS | API transport; notification; CMS/Portfolio; governance/compatibility | `requires_revalidation` — PHASE-03A plan plus bounded shared-transport and B2B HTTP-contract slices merged in PRs #75–#76; broader FE-001/005/006 and INT-011 scope remains open | Existing merged slices retain their bounded evidence; broader implementation remains separately authorized | [PHASE-03A HTTP/command plan](phases/PHASE-03A-http-command-contract-plan.md) | Merged `remediate/phase-03a-shared-transport` and `remediate/phase-03a-backend-contract`; no new branch selected | `9ef645a046518185dcffaf24eaa0136a4f0368cf` | Axios 15-second timeout and one response-less safe-read retry are covered; representative B2B 403/409/422/replay contracts are covered. Raw fetch/download cancellation and timeout, runtime schemas, customer-visible recovery, broader pagination, browser, release, and production evidence remain absent. | 2026-07-29 | Reconcile PRs #75–#76 into the selected-SHA finding record, then authorize only the next exact frontend gap; do not treat the bounded slices as full PHASE-03A closure. |
| 4 - Frontend Engineering and Integration Parity | Auth/client, approved Retail/customer/B2B/CMS journeys | Auth/client; password-policy consumer; Retail/customer; B2B portal; Admin parity; CMS | `partial` / `requires_revalidation` — current snapshot has auth/recovery, shared password-policy consumers, read-only Retail discovery, customer login, Admin B2B/CMS, and shared transport candidates; material gaps are listed below | `ADR-005` authorizes read-only Retail discovery only; Feature 1.4 is bounded password-policy evidence; remaining journey/source work is separately bounded and DR-009/DR-010 remain open | This tracker plus [REMEDIATION_ROADMAP.md](REMEDIATION_ROADMAP.md) | Not selected | `af625942cb64e3a4395f44bd57b74e5bacd7f7b1` fetched current-source reconciliation snapshot; selected RC remains `d04e3f0` | Feature 1.4 password-policy consumers passed 4/4 suites and 11/11 tests on this checkout. Full frontend suite passed 34/35 suites and 235/236 tests; the pre-existing CMS lifecycle contract failure remains. No controlled browser/E2E or release evidence. | 2026-07-29 | Select one bounded frontend task from the register below; preserve decision-blocked Retail transaction/B2B portal scope and do not reopen the Feature 1.4 runtime boundary without a separate task. |
...
| 6 - Testing and Release-Quality Gates | CI, E2E, artifact, static/security, coverage, parity | QA/release gates; package/runtime; dependency risk | `environment_blocked` plus current local failure | `blocked_by_environment` and `blocked_by_decision` for toolchain/threshold policy | [VERIFICATION_MATRIX.md](VERIFICATION_MATRIX.md) | Not selected | `af625942cb64e3a4395f44bd57b74e5bacd7f7b1` fetched current-source reconciliation snapshot | Current full frontend test passed 34/35 suites and 235/236 tests with one CMS static-contract failure. Feature 1.4 records successful source compilation followed by the existing postbuild block on an unconfirmed `REACT_APP_PUBLIC_SITE_URL`; prior local `tsc` stops in `@types/node/ffi.d.ts`; controlled browser/API/role evidence remains absent. | 2026-07-29 | Investigate the CMS contract failure without widening behavior, choose a compatible type-check/toolchain contract under DR-013, and provision the approved release/browser environment before any gate claim. |
...
| `PHASE-01A`–`PHASE-05D` | PHASE-01A records Feature 1.1–1.4 bounded evidence from PRs #79–#81/#84. Feature 1.5 rate-limiter and Feature 1.6 MFA revalidations from PRs #85–#86 inform still-blocked PHASE-01D. PHASE-01B has selected-SHA backend/frontend direct-access evidence on `9ef645a`; PHASE-03A bounded source evidence entered PRs #75–#76. PHASE-04/05 current-source candidates and residual gaps are reconciled below; broader scope remains `blocked_by_decision`, open, environment-blocked, or `requires_revalidation`. | Preserve Feature 1.4 PP-003–PP-006 and the password-rule clarification; complete DR-004/005 from Feature 1.5/1.6 gaps; obtain independent PHASE-01B review; then select one exact frontend row whose gate is satisfied. Do not combine DR-009 Retail transactions with DR-010 B2B portal work. |
...
snapshot is `af625942cb64e3a4395f44bd57b74e5bacd7f7b1`, which matched fetched
`origin/main` when inspected after PRs #84–#86. The selected release candidate
...
it is not finding closure. `Open` means the gap is still directly observable in
current source or tests. `Blocked` means the next result depends on a named
owner decision or controlled environment; it is not a pass.

| Order | Existing phase / task IDs | Finding or gap | Current evidence | Exact remaining task and acceptance boundary | State / gate |
| ---: | --- | --- | --- | --- | --- |
| 1 | `PHASE-04A`; `TASK-04A-01/02` | `FE-006` Customer Order failure recovery | `ClientDashboard.jsx` still uses `.catch(() => {})`, so a failed `GET /orders` becomes indistinguishable from an empty account. | Render a customer-safe visible error with retry and add focused failure/recovery coverage. Do not change the endpoint, projection, session, Order Detail, or legacy read-only lifecycle. | `open`; needs normal bounded source authorization. |
| 2 | `PHASE-04A`; `TASK-04A-01/02` | `FE-004` Admin route fail-closed mapping | `/admin/notifications` is routed through `protectedPage`, but it has no entry in `ADMIN_ROUTE_PERMISSIONS`; an undefined permission is not a fail-closed Admin route contract for every authenticated identity. | Add the explicit permission mapping and customer-to-Admin plus missing-permission negative tests. Preserve backend authorization as the security boundary. | `open`; role matrix is approved, but source work remains bounded separately. |
| 3 | `PHASE-04C`; `TASK-04C-01/02`; `PHASE-06D` | Current CMS lifecycle contract regression, not represented by an audit finding | On `af62594`, full frontend suite passed 34/35 suites and 235/236 tests. `cms-lifecycle.contract.test.js` still expects an outdated indentation-sensitive source substring around `contentApi.rollback`, while current source passes `blockId`, `versionId`, operator reason, and version. This current-checkout result supersedes the Feature 1.4 packet's branch-local all-green full-suite claim for current-main reporting only. | Determine whether the static contract or call signature is stale, preserve operator-supplied rollback reason and optimistic version semantics, then rerun focused and full frontend tests. Do not change runtime behavior merely to satisfy formatting-sensitive text matching. | `open — current test failure`; bounded investigation required before any green-suite claim. |
...
| 19 | `PHASE-06C`; `TASK-06C-01/02` | `FE-ENV-001` release/browser proof | `npm.cmd --prefix frontend run build` compiled successfully, then postbuild rejected the unconfirmed `REACT_APP_PUBLIC_SITE_URL`; no current sitemap/direct-route/browser-role proof was produced. | Use an approved non-secret public origin and controlled API/role/browser topology; record artifact attribution, direct refresh, supported browsers, responsive, and accessibility results. | `blocked_by_environment`; no deployment or go-live implied. |

Recommended next review order is rows 1–3 for small, directly observable
frontend gaps, followed by rows 10–14 for objective accessibility work after
path locks. Rows 7–9, 15–19 must retain their named decision or environment
gates. This ordering is prioritization evidence only, not source authorization.
