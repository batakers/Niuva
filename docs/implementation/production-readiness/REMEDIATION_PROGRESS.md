...
| 1 - Identity, Authentication, Authorization, and Privacy | Auth, role, privacy, file/order boundaries, transaction/payment guard | Session/MFA; recovery; abuse; file/order; transaction; quote; payment; Admin parity | `requires_revalidation` — Feature 1.1–1.5 bounded source/revalidation packets and Feature 1.6 blocked-state audit merged through PRs #79–#86; PHASE-01B selected-SHA evidence passed; independent review and broader decision/environment gates remain | `blocked_by_decision` | [Phase evidence index](phases/README.md); each packet retains its own boundary | PRs #79–#81 and #84–#86; PHASE-01B remains separate | `af625942cb64e3a4395f44bd57b74e5bacd7f7b1` | Customer/Admin Session, Password Recovery, Password Policy, and Rate Limiter evidence entered `main`; Internal MFA was confirmed absent and decision-blocked. PHASE-01B candidate `9ef645a` retains its direct backend/frontend matrix. No MFA implementation, application migration, real provider delivery, deployed-origin, or production acceptance was proven. | 2026-07-29 | Resolve each feature packet's remaining gates, obtain independent PHASE-01B review, finish DR-004 operational limiter inputs, and obtain DR-005 TOTP/key/recovery/support decisions before any MFA source work. |
...
| `PHASE-01A`–`PHASE-05D` | Phase 1 now records merged Feature 1.1–1.3 session/recovery evidence, Feature 1.4 password-policy remediation, Feature 1.5 limiter revalidation, and Feature 1.6 MFA blocked-state audit from PRs #79–#81 and #84–#86. PHASE-01B has selected-SHA backend/frontend direct-access evidence on `9ef645a`; broader scope remains `blocked_by_decision` or `requires_revalidation`. | Close or explicitly disposition the feature-specific open gates below, obtain independent PHASE-01B review, complete DR-004, and obtain DR-005 before any MFA implementation; then record DR-006 through DR-010 as applicable. |
...
authorized separately by `DEC-AUTH-010`. Later bounded feature pull requests
extend the Phase 1 evidence record as follows:
...
| [Feature 1.4 — Password Policy and Hash Migration](phases/FEATURE-1.4-password-policy-hash-migration-remediation.md) | PR #84; merge `29cfae6aabba13c477c2e6e6b2313961c9c30f63` | Password length and hash-write boundaries were reconciled; bcrypt verification remains compatible and Argon2id writes remain explicitly disabled by default. | Production blocklist source/owner, production benchmark evidence, migration activation, rollback-floor evidence, deployment, and go-live remain open. |
| [Feature 1.5 — Authentication Rate Limiter](phases/FEATURE-1.5-auth-rate-limit-revalidation.md) | PR #85; merge `af625942cb64e3a4395f44bd57b74e5bacd7f7b1` | Source/decision/schema review and 23 focused auth tests recorded MongoDB shared counters, HMAC identifiers, account/peer dimensions, generic 429, and `Retry-After`. | Real MongoDB multi-worker concurrency, store-outage policy and fault tests, TTL operational evidence, production proxy topology, monitoring, and retention ownership remain open. |
| [Feature 1.6 — Internal MFA](phases/FEATURE-1.6-internal-mfa-revalidation.md) | PR #86; merge `b06e60d28be337b199ee1f321fe24d84bb163de6` | Current-source audit confirms password-only Admin login still issues an ordinary session and no TOTP, recovery-code, MFA assurance, step-up, or passkey implementation exists. | `DR-005`: TOTP parameters/library, encryption/key custody, pre-auth/step-up contract, recovery/support/break-glass ownership, migration, tests, rollout, environment evidence, and separate passkey phase. |

These merge records prove that reviewed source and evidence entered `main`.
They do not complete roadmap-wide Phase 1, resolve grouped MFA/access-review
findings, or grant migration, deployment, production-readiness, release, or
go-live authority.
