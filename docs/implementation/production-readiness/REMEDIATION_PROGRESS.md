...
| 3 - Backend Business Logic and API Contracts | HTTP envelope, lifecycle, readiness, notification, compatibility/CMS, bounded B2B pagination | API transport; notification; CMS/Portfolio; governance/compatibility; query stability | `requires_revalidation` — Feature 5.1 closes the verified CMS concurrency error seam; Feature 8.3 inventories 21 compatibility endpoint candidates without authorizing retirement; Feature 8.2 is reconciled with current `main` and locally verified for exactly five Admin/B2B list families | Existing merged slices plus separately approved bounded Feature 5.1 and Feature 8.2 authority; Feature 8.3 remains planning evidence | [PHASE-03A HTTP/command plan](phases/PHASE-03A-http-command-contract-plan.md); [Feature 5.1 evidence](phases/FEATURE-5.1-cms-publication-remediation.md); [Feature 8.2 task card](phases/FEATURE-8.2-pagination-task-card.md); [Feature 8.3 register](phases/FEATURE-8.3-compatibility-endpoint-register.md) | Feature 8.2 PR #110 on `feat/backend-pagination`; Feature 8.3 PR #111 is merged evidence | Feature 8.2 implementation `66dbf39`; Feature 8.3 merged through `b16a30b`; reconciled baseline `770a4c3`; selected RC remains `d04e3f0` | Feature 8.2 local review covers route-scoped cursor binding, stable ordering, bounded limits, timezone-aware half-open dates and overflow rejection, deterministic legacy tail, safe error/OpenAPI contracts, and current consumers. Feature 8.3 retains 151 OpenAPI operations and 21 compatibility candidates with unchanged dispositions. No index/query-plan/load, deployed-runtime, external-consumer, release, or production evidence is implied. | 2026-08-03 | Require exact-head CI and merge for PR #110. Decide unresolved Feature 8.3 route/sunset items separately; retain DR-010 portal, migration, release, and production gates. |
| 4 - Frontend Engineering and Integration Parity | Auth/client, approved Retail/customer/B2B/CMS journeys | Auth/client; password-policy consumer; Retail/customer; B2B portal; Admin parity; CMS | `partial` / `requires_revalidation` — current snapshot includes Admin CMS; the current-main-reconciled Feature 8.2 candidate atomically moves current Admin B2B list consumers to the page envelope; remaining journey gaps are listed below | Existing bounded evidence and approved Feature 8.2 source scope only; remaining journey/source work is separately bounded and DR-009/DR-010 remain open | This tracker plus [REMEDIATION_ROADMAP.md](REMEDIATION_ROADMAP.md) and [Feature 8.2 task card](phases/FEATURE-8.2-pagination-task-card.md) | Feature 8.2 PR #110 on `feat/backend-pagination`; Feature 5.1 PR #99; FE-006 merged through PR #89 | Feature 8.2 implementation `66dbf39`; reconciled baseline `770a4c3`; selected RC remains `d04e3f0` | Focused pagination/workbench coverage passes 2/2 suites and 31/31 tests, and the production build succeeds. The Windows full suite passes 36/37 suites and 250/251 tests; its only failure is an unchanged LF-literal CMS source contract under a CRLF checkout. Exact-head Linux CI, controlled browser, release, and production evidence remain absent. | 2026-08-03 | Require exact-head CI and merge for PR #110; preserve decision-blocked Retail transaction/B2B portal scope and select remaining frontend gaps independently. |
...
| 6 - Testing and Release-Quality Gates | CI, E2E, artifact, static/security, coverage, parity | QA/release gates; package/runtime; dependency risk | `environment_blocked` | `blocked_by_environment` and `blocked_by_decision` for toolchain/threshold policy | [VERIFICATION_MATRIX.md](VERIFICATION_MATRIX.md) | Feature 8.2 adds focused B2B cursor/route/consumer coverage on its reconciled candidate; broader gate work not selected | Feature 8.2 reconciled baseline `770a4c3d2d3f3ead0d44f5ac298fac1bb41a0702`; selected RC remains `d04e3f0` | Feature 8.2 local verification: backend 893 passed, 15 skipped, 14 subtests; focused frontend 2/2 suites and 31/31 tests; frontend build, compile, critical lint, 20-file MyPy, 36-file formatting, `pip check`, and diff check passed. The Windows full frontend suite has one unchanged CRLF-sensitive CMS source-contract failure. Local `pip-audit`, controlled browser/API/role, query-plan/load, artifact, threshold, and direct-route evidence remain absent; exact-head CI is required. | 2026-08-03 | Run mandatory exact-head PR gates. Choose the remaining type-check/toolchain contract under DR-013 and provision the approved release/browser environment before any broader gate claim. |
