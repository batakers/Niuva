...
Last updated: 2026-07-31 (Asia/Jakarta)
...
| 3 - Backend Business Logic and API Contracts | HTTP envelope, lifecycle, readiness, notification, compatibility/CMS | API transport; notification; CMS/Portfolio; governance/compatibility | `requires_revalidation` — Feature 5.1 closes the verified CMS concurrency error seam; broader compatibility and consumer scope remains open | Existing merged slices and bounded Feature 5.1 authority only; broader implementation remains separately authorized | [PHASE-03A HTTP/command plan](phases/PHASE-03A-http-command-contract-plan.md); [Feature 5.1 evidence](phases/FEATURE-5.1-cms-publication-remediation.md) | PR #99 on `fix/backend-cms-publication`; earlier transport and Customer Order slices merged | Feature 5.1 baseline `7662a378c3acae6ecc9645b9c471dbb683aac80d`; selected RC remains `d04e3f0` | CMS focus passed 65 tests; three real local replica-set tests prove one-winner publish/rollback and atomic audit-failure rollback; full backend passed 662 tests with 14 skips and 14 subtests. Raw download timeout, runtime schemas, broader pagination, release, and production evidence remain absent. | 2026-07-31 | Review Feature 5.1 independently; retain DR-010 consumer/portal, migration, release, and production gates. |
| 4 - Frontend Engineering and Integration Parity | Auth/client, approved Retail/customer/B2B/CMS journeys | Auth/client; password-policy consumer; Retail/customer; B2B portal; Admin parity; CMS | `partial` / `requires_revalidation` — current snapshot includes Admin CMS and its passing lifecycle contract; remaining journey gaps are listed below | Existing bounded evidence only; remaining journey/source work is separately bounded and DR-009/DR-010 remain open | This tracker plus [REMEDIATION_ROADMAP.md](REMEDIATION_ROADMAP.md) | Feature 5.1 PR #99; FE-006 merged through PR #89 | Feature 5.1 baseline `7662a378c3acae6ecc9645b9c471dbb683aac80d`; selected RC remains `d04e3f0` | Full frontend passes 36/36 suites and 239/239 tests, including the CMS lifecycle contract. This removes the stale formatting-sensitive CMS failure; it is not controlled browser, release, or production evidence. | 2026-07-31 | Preserve decision-blocked Retail transaction/B2B portal scope and select remaining frontend gaps independently. |
...
| 6 - Testing and Release-Quality Gates | CI, E2E, artifact, static/security, coverage, parity | QA/release gates; package/runtime; dependency risk | `environment_blocked` | `blocked_by_environment` and `blocked_by_decision` for toolchain/threshold policy | [VERIFICATION_MATRIX.md](VERIFICATION_MATRIX.md) | Feature 5.1 adds CMS real-transaction coverage to mandatory CI; broader gate work not selected | Feature 5.1 baseline `7662a378c3acae6ecc9645b9c471dbb683aac80d`; selected RC remains `d04e3f0` | Full frontend passes 36/36 suites and 239/239 tests; full backend passes 662 tests with 14 skips and 14 subtests; three CMS real local replica-set tests pass. PR #99 backend, frontend, secret-scan, and disposable `rs-test` transaction jobs passed. Type-check, controlled browser/API/role, artifact, threshold, and direct-route evidence remain absent. | 2026-07-31 | Choose the type-check/toolchain contract under DR-013 and provision the approved release/browser environment before any broader gate claim. |
...
| --- | --- | --- | --- | --- | --- |
| 4.2 Quote lifecycle | `fix/backend-quote-lifecycle`; baseline `735674b`; pull-request delivery authorized | Existing lifecycle, immutable revisions, exact sent/accepted version, acceptance evidence, permissions, and history revalidated. Quote transition, revision, and acceptance events now bind each `operation_id` to an exact canonical command fingerprint; materially different reuse fails `409 operation_id_conflict`. Quote routes declare the shared HTTP error envelope, including fail-closed `503` for revision creation. | Focused backend `66 passed, 1 skipped`; full backend `667 passed, 14 skipped, 14 subtests passed`; real local replica-set B2B integration `5 passed`; adjacent frontend `2` suites and `29` tests passed; critical lint and diff check passed. | Obtain independent review through the authorized pull request. Quote-to-Project remains Feature 4.3; customer Quote access/self-acceptance remains blocked by DR-010; no historical migration, deployment, production-readiness, or go-live is implied. | `source_complete_review_pending`; bounded candidate only. |
