...
| [Feature 5.1 — CMS Publication Lifecycle](phases/FEATURE-5.1-cms-publication-remediation.md) | PR candidate on `fix/backend-cms-publication` | Versioned CMS mutations translate real contention to domain `409`; real replica-set tests prove one-winner publish/rollback and audit-failure atomicity; timezone, permission, immutable snapshot, backend, and frontend regressions pass. | Mandatory disposable `rs-test` PR evidence, Migration 007, controlled browser/release environment, deployment, monitoring, production readiness, and go-live remain separately gated. |
