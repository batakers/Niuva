...
| 8 - Reliability, Performance, and Observability | Readiness, telemetry, workers, timeout, capacity, performance | Observability/background; release/readiness; maintainability/performance | `partial` — current-main local/source audit revalidates truthful liveness/readiness, notification claim/lease/retry/exhaustion, bounded worker drain, redacted provider-neutral telemetry, required indexes/pagination, and the bounded GET-only load probe; external telemetry, alerts, representative capacity, and deployed failure drills remain open | Local source/test authority under `DEC-READY-01`, `DEC-OBS-001`, `DEC-DATA-003`, `ADR-001`, `ADR-002`, and `ADR-005`; production/provider/deployment authority remains blocked | [current-main runtime audit](phases/RUNTIME-RELIABILITY-CURRENT-MAIN-REVALIDATION-2026-08-14.md) | `audit/backend-runtime-reliability`, stacked behind PR #250 | Runtime baseline `15b759a`; audit stack base `c53f06d` | Focused local matrix `223 passed`, including dependency unavailable/timeout, stale worker, storage/email failure boundaries, fenced leases, deterministic exhaustion, redaction/resource bounds, schema/index and pagination/load checks. No external target or provider was contacted. | 2026-08-14 | Deliver the stacked audit; separately approve telemetry destination, SLO/on-call ownership, production worker/deployment topology, representative capacity target, and controlled failure/load drills. |
