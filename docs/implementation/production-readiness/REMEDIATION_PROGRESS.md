...
| 1 - Identity, Authentication, Authorization, and Privacy | Auth, role, privacy, file/order boundaries, transaction/payment guard | Session/MFA; recovery; password policy/hash migration; abuse; file/order; transaction; quote; payment; Admin parity | `requires_revalidation` — PR #95 (shared transaction executor), PR #96 (exact Quote-line identity), and PR #93 (file authorization/security) are merged into current `main`; broader Phase 1 decision and production gates remain open. | Existing bounded implementation authority only; broader Phase 1 remains `blocked_by_decision` | [Phase evidence index](phases/README.md); [3.1–3.2 register below](#transaction-and-commercial-integrity-feature-register--30-july-2026) | Merged evidence in `main`; no new implementation branch selected | `57de1f36e297e250705e8c47df5bef6b8da86fc9` | Current-main revalidation: full backend `653 passed, 13 skipped, 14 subtests`; auth/authorization `92 passed, 1 skipped`; transaction/Quote `53 passed, 2 skipped`; file/readiness/migration contracts `94 passed, 2 skipped`; real local replica-set transaction/session coverage `21 passed` without migration execution. | 2026-07-31 | Preserve password, MFA, migration, historical-data, provider, deployment, and production gates; select any next source task separately. |
...
| 3 - Backend Business Logic and API Contracts | HTTP envelope, lifecycle, readiness, notification, compatibility/CMS, bounded B2B pagination | API transport; notification; CMS/Portfolio; governance/compatibility; query stability | `requires_revalidation` — Feature 5.1 closes the verified CMS concurrency error seam; Feature 8.2 is source-complete on its stacked candidate for exactly five Admin/B2B list families; broader compatibility, raw-download, schema, and consumer scope remains open | Existing merged slices plus separately approved bounded Feature 5.1 and Feature 8.2 authority only | [PHASE-03A HTTP/command plan](phases/PHASE-03A-http-command-contract-plan.md); [Feature 5.1 evidence](phases/FEATURE-5.1-cms-publication-remediation.md); [Feature 8.2 task card](phases/FEATURE-8.2-pagination-task-card.md) | Feature 8.2 PR #110 on `feat/backend-pagination`, stacked on Feature 8.1 PR #109; earlier slices retain their recorded branches | Feature 8.2 implementation `66dbf39`; baseline `572e0c0dcd60ac847b1f100ea795755b9f484579`; selected RC remains `d04e3f0` | Feature 8.2 focused backend passed 33 tests; full backend passed 705 tests with 14 skips and 14 subtests. Stable ordering, cursor/filter binding, bounded limits, timezone-aware half-open dates, legacy tail, error/OpenAPI, and current consumer contracts are covered locally. No index/query-plan/load, release, or production evidence exists. | 2026-08-02 | Obtain independent review and green CI for PR #110; preserve PR order #106, #109, then #110 and rerun current-main/CI verification before merge. Retain DR-010 portal, migration, release, and production gates. |
| 4 - Frontend Engineering and Integration Parity | Auth/client, approved Retail/customer/B2B/CMS journeys | Auth/client; password-policy consumer; Retail/customer; B2B portal; Admin parity; CMS | `partial` / `requires_revalidation` — current snapshot includes Admin CMS; the Feature 8.2 candidate atomically moves current Admin B2B list consumers to the page envelope; remaining journey gaps are listed below | Existing bounded evidence and approved Feature 8.2 source scope only; remaining journey/source work is separately bounded and DR-009/DR-010 remain open | This tracker plus [REMEDIATION_ROADMAP.md](REMEDIATION_ROADMAP.md) and [Feature 8.2 task card](phases/FEATURE-8.2-pagination-task-card.md) | Feature 8.2 PR #110 on `feat/backend-pagination`, stacked on PR #109; Feature 5.1 PR #99; FE-006 merged through PR #89 | Feature 8.2 implementation `66dbf39`; baseline `572e0c0dcd60ac847b1f100ea795755b9f484579`; selected RC remains `d04e3f0` | Full frontend passes 37/37 suites and 249/249 tests; production build succeeds. This is local source evidence, not controlled browser, release, or production evidence. | 2026-08-02 | Review and revalidate PR #110 after its bases merge; preserve decision-blocked Retail transaction/B2B portal scope and select remaining frontend gaps independently. |
...
| 6 - Testing and Release-Quality Gates | CI, E2E, artifact, static/security, coverage, parity | QA/release gates; package/runtime; dependency risk | `environment_blocked` | `blocked_by_environment` and `blocked_by_decision` for toolchain/threshold policy | [VERIFICATION_MATRIX.md](VERIFICATION_MATRIX.md) | Feature 8.2 adds focused B2B cursor/route/consumer coverage on its candidate; broader gate work not selected | Feature 8.2 baseline `572e0c0dcd60ac847b1f100ea795755b9f484579`; selected RC remains `d04e3f0` | Feature 8.2 local verification: backend 705 passed, 14 skipped, 14 subtests; frontend 37/37 suites and 249/249 tests; frontend build, compile, critical lint, focused formatting, focused MyPy, and diff check passed. PR CI, controlled browser/API/role, query-plan/load, artifact, threshold, and direct-route evidence remain absent. | 2026-08-02 | Run mandatory PR gates, then revalidate after the stacked bases merge. Choose the remaining type-check/toolchain contract under DR-013 and provision the approved release/browser environment before any broader gate claim. |
...
| 4 | `PHASE-03A`; `TASK-03A-01/02`; `PHASE-04A`; Feature 8.2 | Residual `FE-001`, `FE-005`, `INT-011` transport/schema/envelope work | PRs #75–#76 provide Axios timeout/retry behavior. Feature 8.2 locally validates the strict `{items,next_cursor}` envelope and explicit continuation for the five approved Admin/B2B lists. Raw `fetchFile`/download paths still have no common timeout/abort policy; no general runtime-schema adapter exists; pagination for all other route families remains unchanged. | Review and merge Feature 8.2 in stack order. Separately freeze the next critical response schema and invalid-data state plus a bounded raw-download timeout/cancellation contract. Do not expand pagination, retry commands, or change refresh/CSRF/download semantics implicitly. | `partial` / `source_complete_review_pending` for bounded Feature 8.2; remaining schema, download, and other-route scope still requires separate authority. |
