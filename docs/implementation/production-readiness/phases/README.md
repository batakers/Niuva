...
| 7.2 Worker Topology | [task card](FEATURE-7.2-worker-topology-task-card.md); [decision package](FEATURE-7.2-worker-topology-decision-package.md) | PR #107 merged as `fe1d8a0` | High-level Option B plus bounded lease/worker values (15s operation, 5s acknowledgement, 40s margin, 60s lease, 30s renewal, concurrency 1, claim-ahead 0, 30s drain) are approved by Faiz on 4 August 2026 UTC (5 August 2026 Asia/Jakarta); `decision_blocked` remains for SLO, capacity, telemetry, retention/access, alert, and evidence values. Source implementation remains unauthorized. |
| 7.3 Backend Observability | [task card](FEATURE-7.3-backend-observability-task-card.md); [decision package](FEATURE-7.3-backend-observability-decision-package.md) | PR #108 and worksheet PR #133 merged; follow-up proposal pending | High-level JSON Lines to stdout/stderr, no external provider, and the bounded Feature 7.2 worker values are approved by Faiz on 4 August 2026 UTC (5 August 2026 Asia/Jakarta). A provider-neutral candidate contract, concrete sandbox baseline, and approval worksheet cover classification/redaction, retention/access, metrics/SLI/cardinality, capacity, exporter outage, SLO/error budget, alerts, responders, and evidence; `decision_blocked` remains until the baseline is explicitly approved. Source implementation/provider activation remain unauthorized. |
