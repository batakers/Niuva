...
| 3.4A Retail Order Contract Hardening | [task card](FEATURE-3.4A-retail-order-contract-hardening-task-card.md); [evidence](FEATURE-3.4A-retail-order-contract-hardening.md) | PR pending on `feat/backend-retail-order-contract-hardening` | Provider-neutral cart/snapshot, idempotency, basic lifecycle, append-only audit, and concurrency contracts are locally verified while all Retail transaction routes remain inactive. Full 3.4 activation, persistence, reservation, payment, provider, migration, deployment, and production gates remain open. |
