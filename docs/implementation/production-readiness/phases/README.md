...
| 7.2 Worker Topology | [task card](FEATURE-7.2-worker-topology-task-card.md); [decision package](FEATURE-7.2-worker-topology-decision-package.md) | PR #107 open; Project Owner approved the recommended planning direction and authorized commit/push/PR on 2 August 2026 | `decision_blocked`: Operations/SRE/Security acceptance and numerical timing, telemetry/SLO, capacity, destination, and accountable-owner inputs remain pending under DR-014; source implementation and merge are not authorized. |
