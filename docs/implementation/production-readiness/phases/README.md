...
| 5.1 CMS Publication Lifecycle | [remediation](FEATURE-5.1-cms-publication-remediation.md) | PR #99 open; backend/frontend/secret/transaction CI passed | Concurrent publish/rollback conflicts and audit-failure atomicity are covered by local and disposable-CI replica-set tests; timezone and permission contracts pass. Migration 007, production topology, deployment, and broader DR-010 consumer decisions remain open. |
...
| 7.2 Worker Topology | [task card](FEATURE-7.2-worker-topology-task-card.md); [decision package](FEATURE-7.2-worker-topology-decision-package.md) | PR #107 merged as `fe1d8a0` | Project Owner-approved planning direction is recorded; `decision_blocked` remains for Operations/SRE/Security acceptance and numerical timing, telemetry/SLO, capacity, destination, and accountable-owner inputs under DR-014. Source implementation remains unauthorized. |
| 7.3 Backend Observability | [task card](FEATURE-7.3-backend-observability-task-card.md); [decision package](FEATURE-7.3-backend-observability-decision-package.md) | PR #108 open; reconciliation review complete and merge authorized 3 August 2026 | `decision_blocked`: provider-neutral logging, metrics, alerting, timeout, worker, transaction, retention, SLO, responder, and capacity decisions remain pending under DR-014; source implementation/provider activation remain unauthorized. |
