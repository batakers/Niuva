...
| 1.2 Admin Session | [Read-only revalidation](FEATURE-1.2-admin-session-revalidation.md); [bounded completion](FEATURE-1.2-admin-session-completion.md) | #80 merged; #94 merged into open #93 branch | Forced re-login cross-tab browser evidence and disposable replica-set Migration 009 drill complete. AS-001 production HTTPS/proxy, restore, monitoring, cutover, deployment, activation, and observation evidence remain open. |
...
| 2.4 File Authorization and Security | [remediation](FEATURE-2.4-file-security-remediation.md) | #93 open | Bounded local/CI hardening under review covers size/signature validation, database ownership and domain scope, active-state downloads, safe media, query-token rejection, and compensation. Production provider/scanner, retention/quota, backup/restore, owners, reconciliation, deployment, and go-live remain open. |
| 3.1 Shared Transaction Executor | [main tracker notes](../REMEDIATION_PROGRESS.md#transaction-and-commercial-integrity-feature-register--30-july-2026) | #95 open | Green CI, but actionable runbook and catalog concurrency/slug-race review findings remain. Verify, fix, and reverify before merge; no production topology or migration claim. |
