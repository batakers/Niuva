...
| 1.2 Admin Session | [Read-only revalidation](FEATURE-1.2-admin-session-revalidation.md); [bounded completion](FEATURE-1.2-admin-session-completion.md) | #80 merged; completion PR open | Forced re-login cross-tab browser evidence and disposable replica-set Migration 009 drill complete. AS-001 production HTTPS/proxy, restore, monitoring, cutover, deployment, activation, and observation evidence remain open. |
