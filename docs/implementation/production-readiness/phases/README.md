...
| 7.2 Worker Topology | [task card](FEATURE-7.2-worker-topology-task-card.md); [decision package](FEATURE-7.2-worker-topology-decision-package.md) | PR #107 merged as `fe1d8a0` | High-level Option B, no external queue/scheduler provider, at-least-once delivery, and bounded shutdown are approved by Faiz; `decision_blocked` remains for numerical timing, telemetry/SLO, capacity, destination, and other values. Source implementation remains unauthorized. |
| 7.3 Backend Observability | [task card](FEATURE-7.3-backend-observability-task-card.md); [decision package](FEATURE-7.3-backend-observability-decision-package.md) | PR #108 merged as `b336198` | High-level JSON Lines to stdout/stderr with no external provider is approved by Faiz; `decision_blocked` remains for schema, metrics, alerting, retention, SLO, responder, capacity, and destination values. Source implementation/provider activation remain unauthorized. |
