...
`2ccb340a4694c8fe466531a666514edd5bd9c1e2` (`2ccb340`), Git tree
`e8893474a388727365a387ee7a11491dc55df842`, fetched on 6 August 2026
(Asia/Jakarta).

**Merge parents:** `1a99bd7cd56500bb3ea08a892f01ffc0ed22ec79` and
`cfed53be307d4a4f9cc2908eacc277a78a9010de`.

**Worktree:** `C:\tmp\niuva-pr-196-197-current-head-fix-20260806`, clean at task start, with
`HEAD = origin/main` and `origin/main...HEAD = 0/0`.

The requested baseline `c7452b8` is stale. The immediately prior current-main
observation was `1a99bd7` after PR #198. PR #199 then entered `main` with the
current-head documentation re-anchor. This packet records the subsequent
repository state only; it does not select `2ccb340` as a release candidate or
...
| Current `origin/main` | `2ccb340a4694c8fe466531a666514edd5bd9c1e2` (`2ccb340`) | Point-in-time observation only; DR-001 remains open |
| Git tree | `e8893474a388727365a387ee7a11491dc55df842` | Source-tree identity, not a published artifact identity |
| Merge parents | `1a99bd7cd56500bb3ea08a892f01ffc0ed22ec79` and `cfed53be307d4a4f9cc2908eacc277a78a9010de` | Git ancestry only; current merge is PR #199 |
| Requested-baseline delta | `c7452b8..2ccb340`: 139 commits, 95 paths, 10010 additions, 212 deletions | Scope indicator, not readiness completion |
...
merged and reconcile the current-head documentation. PRs #196 and #197 are
stale, conflicting branches based on the older `9472537` head and are not part
of this tree.
PR #189's numeric bundle budget remains source/config state while canonical
DR-013 remains Open.

## 3. Changed-path boundaries after PR #199

The exact `1a99bd7..2ccb340` delta from PR #199 is **four documentation paths,
100 additions, and 88 deletions**. The preceding `9b30441..1a99bd7` interval
from PR #198 is also four documentation paths; the earlier `9472537..9b30441`
interval from PR #185 remains five documentation paths. Earlier #189 and #195
intervals remain separately classified historical evidence. No application runtime,
...
| Check | Result | Limit |
| --- | --- | --- |
| Fresh fetch and exact worktree | Passed; `HEAD = origin/main = 2ccb340`, clean, `0/0` divergent | Does not prove external environment state |
| Current main `quality-gates` | Passed at SHA `2ccb340`; run [`31100812342`](https://github.com/batakers/Niuva/actions/runs/31100812342) | CI evidence only; not staging or production evidence |
| Post-merge changed-path audit | Passed; `1a99bd7..2ccb340` is four documentation paths; earlier #185/#189/#195/#198 intervals are separately classified | Does not re-run every historical packet's external gate |
...
- **DR-001:** Open. `2ccb340` is observed current `main`, not a selected
...
- **PRs #196/#197:** Open, stale, and conflicting after #198/#199; their passing
...
The main risks are evidence staleness, treating `2ccb340` as an unapproved
