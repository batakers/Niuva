...
| Affected areas | `scripts/collect_backend_quality_baseline.py`, `scripts/verify_pytest_evidence.py`, the shared transaction executor if retry evidence reproduces a defect, focused tests, CI/evidence documentation, and primary readiness trackers; workflow changes only if an objective fail-open defect is reproduced. |
