...
| QUALITY-001 | Backend static-quality debt (flake8/mypy/black/isort) | Backend | **In progress** | PR #258: black/isort now 0 remaining. PR #262: `.flake8` config fix reduced flake8 from 2,046 to 433 genuine findings. PR #263 (part 2): fixed F401/F841/W391. PR #264 (part 3): fixed E301/E302/E306. PR #265 (part 4): fixed F402 (import-shadowing risk, investigated first, not a live bug). This PR (part 5): fixed all 58 E402 findings across 4 files (`server.py`: 55, `conftest.py`/`test_auth_recovery_transaction_integration.py`/`test_identity_access_migration.py`: 1 each) — investigated first and confirmed every case is intentional (env vars must be loaded via `load_dotenv()`, or test bootstrap state set, before the imports that depend on them run), so no import was reordered; instead added/completed `# noqa: E402` annotations, and re-wrapped one line that grew past 88 columns as a result. flake8 now reports 0 for E301/E302/E306/E402/F401/F841/F402/W391. Only 274 mypy findings remain open, for further bounded PRs. Pre-existing `black`/`isort` debt and one pre-existing F401 (`fastapi.Form`/`Header` in `server.py`, already fixed on the still-unmerged #263) are left untouched, not duplicated here |
...
| (this branch) | E402 import-order noqa annotations (QUALITY-001, part 5) | `chore/niuva-backend-flake8-e402` |
