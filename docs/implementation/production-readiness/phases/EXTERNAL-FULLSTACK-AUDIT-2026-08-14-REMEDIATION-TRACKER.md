...
| DESIGN-001 | NDS migration incomplete (legacy fonts, `transition-all`, side-border) | Frontend | **Open — legacy-font removal blocked, not a bug** | Investigated whether Poppins/JetBrains Mono could simply be deleted as "leftover legacy code": they cannot. `DEC-UX-004` explicitly rules that "Poppins, Inter, hosted JetBrains Mono... may continue only as recorded compatibility contracts until source consumers are migrated; they receive no new consumer" — and a dedicated test (`defines NDS typography roles without removing compatibility delivery`) asserts `--font-family-sans: 'Poppins'` and the Google Fonts import stay present. Same pattern as the `vaul` finding in DEP-001. What IS still open: 27 files (nearly the entire Admin/Operations surface, plus `ContactPage.jsx`) still consume the compatibility font roles instead of the NDS 2.0 tokens. Migrating them needs a PM/design decision on sequencing, per `DEC-UX-004`'s one-surface-at-a-time migration requirement. `transition-all`/side-border findings not yet investigated |
...
| [#268](https://github.com/batakers/Niuva/pull/268) | Normalize GSAP motion timing to approved grammar (MOTION-001) | `fix/niuva-frontend-gsap-motion-timing` | **Merged** |
| [#269](https://github.com/batakers/Niuva/pull/269) | Legacy-font cleanup investigation (DESIGN-001, docs only) | `docs/niuva-legacy-font-cleanup-investigation-20260816` | Open |
