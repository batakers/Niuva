...
| QUALITY-001 | Backend static-quality debt (flake8/mypy/black/isort) | Backend | **In progress** | PR #258: black/isort now 0 remaining. PR #262: `.flake8` config fix reduced flake8 from 2,046 to 433 genuine findings. PR #263 (part 2): fixed F401/F841/W391. PR #264 (part 3): fixed E301/E302/E306 (23 findings, pure whitespace). This PR (part 4): fixed all 6 F402 findings in `notification_schema_report.py` — investigated first (AST-checked every function that shadows the module-level `dataclasses.field` import with a `for field in ...:` loop) and confirmed none of them ever call `field(...)` in that scope, so this was a naming-hygiene risk, not a live bug; fixed by renaming the loop variable to `field_name`. flake8 now reports 0 for E301/E302/E306/F401/F841/F402/W391. Remaining: E402 (58, module-level-import-not-at-top — needs review since reordering can change execution order) and all 274 mypy findings, still open for further bounded PRs. Pre-existing `black`/`isort` formatting debt in touched files (unrelated to these edits) is left for PR #258 to resolve on merge, not duplicated here |
