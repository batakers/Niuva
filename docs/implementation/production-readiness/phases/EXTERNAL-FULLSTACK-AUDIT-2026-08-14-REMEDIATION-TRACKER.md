...
| QUALITY-001 | Backend static-quality debt (flake8/mypy/black/isort) | Backend | **In progress** | PR #258: black/isort now 0 remaining. This PR: `.flake8` config fix reduces flake8 from 2,046 to 433 genuine findings (1,948 were a line-length config mismatch, not real debt). mypy (274) and the remaining 433 flake8 findings still open, to be worked in further bounded PRs |
...
| (this branch) | `.flake8` config fix (QUALITY-001, part 1) | `chore/niuva-backend-flake8-line-length-config` |
