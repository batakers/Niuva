...
| DESIGN-001 | NDS migration incomplete (legacy fonts, `transition-all`, side-border) | Frontend | **Open — legacy-font removal blocked, not a bug** | Investigated on 2026-08-16 whether Poppins/JetBrains Mono could simply be deleted as "leftover legacy code": they cannot. `DEC-UX-004` explicitly rules that "Poppins, Inter, hosted JetBrains Mono, old palette values, and old aliases may continue only as recorded compatibility contracts until source consumers are migrated; they receive no new consumer" — and a dedicated test (`defines NDS typography roles without removing compatibility delivery`) asserts `--font-family-sans: 'Poppins'` and the Google Fonts import stay present. Same pattern as the `vaul` finding in DEP-001: looks like dead code, is actually intentional scaffolding. What IS still open: 27 files (nearly the entire Admin/Operations surface, plus `ContactPage.jsx`) still consume the compatibility font roles (`font-sans`/`font-display`/`font-body`/`font-mono`/`font-technical`) instead of the NDS 2.0 tokens (Mona Sans/Bona Nova). Migrating them is a legitimate follow-up, but per `DEC-UX-004` surfaces migrate one at a time with design review each time — not something to bulk-execute without a PM/design decision on sequencing, especially this close to a demo where those are the most-used screens. `transition-all`/side-border findings not yet investigated |
