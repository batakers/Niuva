...
| DR-003 | Confirm the bounded auth recovery/session scope on the selected baseline. | Newer default-branch Phase 1/2 work is evidence of change, not closure of the audit snapshot. | Auth implementation planning and Phase 1 exit. | DEC-AUTH-003/004/005/010; recovery/session packets and runbooks; selected-SHA tests. | Security/identity owner; explicit included/excluded scope and rollout boundary. | Faiz approved the original bounded selected-SHA revalidation scope on 2026-07-29 for password recovery and Admin session only. Customer Session was later approved separately under `DEC-AUTH-010`; Feature 1.1, 1.2, and 1.3 evidence entered `main` through PRs #79, #80, and #81. These merges do not authorize real email, shared/staging/production work, Argon2-write enablement, migration application, deployment, activation, or go-live, and do not resolve DR-004/005 or the Phase 1 exit. |
