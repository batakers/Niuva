...
| V-02-02 | 2 | Migration/backup/restore | Prove dry run, apply, second run, rollback, and restore on an isolated replica set. | Approved per-migration plan, encrypted backup custody where applicable, restore owner, and DR-012 target/topology evidence. | Aggregate-only evidence; backup restore succeeds; owned indexes/markers and historical-record preservation are verified. | Planning package ready for owner/reviewer review; execution remains blocked by DR-012 and target/topology evidence. |
