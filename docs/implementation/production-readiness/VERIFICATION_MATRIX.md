...
| V-00-03 | 0/2 | Migration/live-schema | Prove that no migration begins without backup, dry run, validation, rollback, owner, and stop conditions. | Named target and migration authority. | Per-migration preflight/rollback plan; no mutation is run merely to fill the tracker. | Local read-only dry-runs for 007–010 completed without marker/index/data changes after a bounded Migration 010 CLI fix candidate. Apply, rollback, backup/restore, and isolated/shared/production execution remain blocked by DR-012. |
...
| V-01-04 | 1 | Transaction/idempotency/quote/payment | Prove replay, expected-version conflict, duplicate operation, payment capability denial, and fail-closed transaction behavior. | ADR-001/ADR-003; real disposable replica set; exact quote-line decision. | One-effect behavior, controlled 409/503 responses, and no non-atomic fallback. | PRs #95–#96 are merged. Current-main focused transaction/Quote tests passed `53` with two environment-gated skips; a real local replica-set transaction/session suite passed `21` without migration execution. The run used unique auto-dropped databases on local `rs0`, not the tracked disposable `rs-test`, so controlled-environment and production claims remain open. No payment/provider claim. |
| V-02-01 | 2 | Schema/reference/notification | Prove a non-destructive schema/reference report and duplicate/orphan handling plan before migration. | `DEC-DATA-002`, `DEC-DATA-003`, and representative isolated data. | Report identifies zero unexplained ambiguity or stops safely; no raw customer data in output. | Quote-line aggregate-only report source and tests are merged through PR #96. Execution remains pending an explicitly approved representative isolated dataset; notification scope remains separate. |
| V-02-02 | 2 | Migration/backup/restore | Prove dry run, apply, second run, rollback, and restore on an isolated replica set. | Approved per-migration plan, encrypted backup custody where applicable, restore owner, and DR-012 target/topology evidence. | Aggregate-only evidence; backup restore succeeds; owned indexes/markers and historical-record preservation are verified. | Partial pass on bounded disposable-local backup/restore only: `rs-test` source-to-second-database restore passed 4/4 with SHA-256 custody, content-identical comparison, historical Quote/Project reference and Decimal128 preservation, and verified database/snapshot cleanup. No migration ran; PHASE-02B is merged through PR #112, while per-migration apply/rerun/rollback, independent review, and operational/shared DR-012 gates remain open. |
| V-03-01 | 3 | API transport/envelope | Prove documented error, pagination, invalid-data, timeout, retry, cancellation, and idempotency behavior. | API governance decision and stable fixtures. | Contract tests cover normal, malformed, offline/timeout, replay, conflict, permission, and service-unavailable cases. | PHASE-03A planning packet and bounded transport policy exist; source verification remains pending exact route/file and implementation authorization. |
| V-03-02 | 3 | Notification/readiness/CMS | Prove truthful readiness and governed lifecycle/compatibility behavior. | Service-health and remaining CMS/portfolio consumer decisions. | Health endpoint distinguishes dependency failure; publish/rollback/recipient logic has explicit allowed states and tests. | Partial: Feature 5.1 proves CMS publish/rollback permission, timezone, immutable snapshot, one-winner concurrency, and audit-failure atomicity; notification/readiness and broader consumer evidence remain open. |
...
| V-06-01 | 6 | QA/release/artifact | Prove clean install, backend/frontend tests, critical E2E, artifact output, static/coverage/security/dependency/performance signals, and expected-skip enforcement. | Chosen package/runtime policy, CI services, public origin, browser engines, isolated database. | CI records exact SHA/runtime/environment; unexpected skip, zero executed tests, or absent required artifact is a failure, not a pass. | Current-main repository audit closes untracked-virtualenv pollution and empty-JUnit acceptance; hermetic local evidence passes and required hermetic/transaction/public-smoke artifacts are fail-closed. Exact-head transaction CI, external public/Admin runs, Admin browser artifact/checksum enforcement, threshold/legal/lifecycle ownership, and production-like evidence remain environment/decision blocked. See the [quality evidence packet](phases/QUALITY-EVIDENCE-CURRENT-MAIN-REVALIDATION-2026-08-14.md). |
...
| V-10-01 | 10 | All applicable P0/P1 and accepted risks | Prove a production candidate is ready for an explicit human readiness decision. | Exact candidate, production-like environment, all Phase 0-9 exit evidence. | Full regression, security review, migration dry run, backup/restore, rollback, monitor/alert and handoff evidence are current and attributable. | PHASE-10C planning packet defines exact-SHA migration, backup/restore, transaction/recovery-failure, artifact-rollback, and data-rollback evidence, but performs none of it. Verification remains blocked by PHASE-10A, DR-012, an approved isolated target/window, immutable artifacts, accepted PHASE-02C evidence, and independent review. |
