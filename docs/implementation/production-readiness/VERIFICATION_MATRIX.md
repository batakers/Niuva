...
| V-01-04 | 1 | Transaction/idempotency/quote/payment | Prove replay, expected-version conflict, duplicate operation, payment capability denial, and fail-closed transaction behavior. | ADR-001/ADR-003; real disposable replica set; exact quote-line decision. | One-effect behavior, controlled 409/503 responses, and no non-atomic fallback. | Candidate evidence exists in PRs #95–#96. PR #95 has green CI but unresolved actionable review findings. PR #96 has 68 passing real replica-set tests and green CI; substantive automated review remains pending. No payment/provider or production claim. |
| V-02-01 | 2 | Schema/reference/notification | Prove a non-destructive schema/reference report and duplicate/orphan handling plan before migration. | `DEC-DATA-002`, `DEC-DATA-003`, and representative isolated data. | Report identifies zero unexplained ambiguity or stops safely; no raw customer data in output. | Quote-line aggregate-only report source and tests exist in PR #96. Execution remains pending an explicitly approved representative isolated dataset; notification scope remains separate. |
