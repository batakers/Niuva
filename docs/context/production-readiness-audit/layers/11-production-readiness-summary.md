...
| Quote-line identity, references, uniqueness and retention: `BE-004`, `DB-004`, `DB-013` | 03, 04 | P1 | `BE-004`/`DB-004` resolved in source; broader `DB-013` retention remains open | 100% for Quote-line source scope | No for source scope; historical execution separately gated | Yes pending production data evidence | Approved isolated target, reviewed aggregate report, retention owners | Phase 1 / later data gate |
