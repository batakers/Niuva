...
| `PRODUCT.md` | Approved Baseline | Product purpose, principles, locked and deferred decisions | Approved summary baseline; subordinate to more specific approved decisions and ADRs | Fast product orientation before consolidation is approved | Overriding a more specific approved decision or ADR |
| `AGENTS.md` | Approved Baseline | Repository implementation guardrails | Approved implementation guardrail; subordinate to more specific approved requirements, decisions, and ADRs | Safe coding workflow, privacy, data, role, and implementation boundaries | Creating new business, provider, policy, or visual decisions |
...
| `memory/PRD.md` | Context Only | Pointer to active PRD | No independent requirement authority | Locating the pre-consolidation PRD | Product decisions or new planning after Master Spec approval |
...
| `doc/decisions/ADR-003-retail-payment-orchestration-boundary.md` | Approved with Open Decisions | Retail payment architecture | Technical authority for provider-neutral online-payment boundary only | Core payment contract, adapter separation, idempotency, refund/reconciliation boundaries | Selecting gateway, enabling manual transfer, Finance activation, or go-live |
...
| `doc/Company profile PT Niuva_compressed.pdf` | Supporting Reference | Company positioning, services, projects, and contact facts | Factual source material; approval metadata is not recorded | Verifying company language and project facts | Inventing outcomes, metrics, clients, or current product requirements |
| `doc/brand/BRAND_WEBSITE_AUDIT.md` | Supporting Reference | Public website audit | Analysis and recommendation only | Understanding composition, logo, typography, motif, and photography conflicts | Treating recommendations as approval without a decision record |
| `doc/brand/BRAND_APPROVAL_DECISIONS.md` | Approved Decision | Isolated Homepage prototype scope | Approved only for prototype work | Prototype constraints, protected foundations, and decision evidence | Production rollout or final Homepage direction where later decision exists |
| `doc/brand/HOMEPAGE_PROTOTYPE_REVIEW.md` | Context Only | Prototype comparison evidence | Review evidence only | Understanding comparison, QA, strengths, risks, and recommendation | Production authority or route rollout |
| `doc/brand/HOMEPAGE_PROTOTYPE_DECISION.md` | Approved Decision | Production Homepage planning direction | Specific approved Homepage design decision; implementation still not authorized | Experimental Editorial Hybrid, typography, U-curve, motion, composition, and protected foundations | Detailed Retail/B2B navigation or redesigning other routes |
