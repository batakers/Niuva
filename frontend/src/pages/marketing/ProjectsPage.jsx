...
import { getLocalizedField, useI18n } from "@/i18n";
import {
  BrandButton,
  ...
  const projects = useMemo(
    () =>
      profileContent.projects.map((project) => {
        ...
        return {
          ...project,
          title: getLocalizedField(match, "title", locale) || project.title,
          body: getLocalizedField(match, "description", locale) || project.body,
          ...
          imageAlt: match?.images?.[0]
            ? `${locale === "en" ? "Documentation of" : "Dokumentasi"} ${getLocalizedField(match, "title", locale) || project.title}`
            : project.imageAlt,
          imageFit: match?.images?.[0] ? "cover" : project.imageFit,
          client: match?.client,
        };
      }),
    [apiProjects, locale, profileContent.projects]
  );
