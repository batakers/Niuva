        ...
        <div className="p-6 space-y-8">
          ...
          <div className="grid sm:grid-cols-2 gap-6">
            ...
            <div className="border border-border p-4 bg-background flex flex-col justify-between">
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                <Button type="button" onClick={() => downloadFile(order.file?.storage_path, order.file?.original_filename).catch((err) => toast.error(formatApiError(err.response?.data?.detail)))} size="sm" variant="outline" data-testid="admin-download-design" className="rounded-none border-border hover:border-primary uppercase tracking-widest font-mono text-[10px] h-8 px-3">
          ...
          {order.payment && (
            <div className="border border-border p-6 bg-surface-2">
              ...
              <Button type="button" variant="outline" onClick={() => downloadFile(order.payment.proof?.storage_path, order.payment.proof?.original_filename).catch((err) => toast.error(formatApiError(err.response?.data?.detail)))} className="mb-4 w-full rounded-none border-border font-mono text-xs uppercase tracking-widest">
                <Download className="h-4 w-4" /> DOWNLOAD_PAYMENT_PROOF
              </Button>
