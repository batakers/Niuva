...
export function BrandPage({ children, className }) {
  ...
  return (
    <div ref={pageRef} className={cn("brand-page w-full overflow-x-hidden", className)}>
      {localizePublicContent(children, locale)}
    </div>
  );
}
    ...
    <section className={cn("marketing-page-hero relative overflow-hidden", className)} data-marketing-section="hero">
      ...
      >
        ...
        <div className="brand-reveal relative z-10 min-w-0">
          {visualContent || (
            <RoundedVisualFrame title={locale === "en" ? "From research to realization" : "Dari riset ke realisasi"} kicker="Niuva Inovasi Utama">
...
export function ContactSummary({ className, contact, showMapLink = false }) {
  const { locale } = useI18n();
  const items = [
    { label: locale === "en" ? "Location" : "Lokasi", value: contact.location, href: showMapLink ? contact.mapsHref : undefined },
  ...
  return (
    <div className={cn("grid gap-4 md:grid-cols-3", className)}>
      {items.map((item, index) => (
        <div key={`contact-channel-${index}`} className="brand-reveal relative min-h-[170px] overflow-hidden rounded-card border border-border-default bg-surface-default p-5 sm:p-6">
...
function RequiredLabel({ children }) {
  ...
  return (
    <>
      {children} <span className="font-normal text-text-secondary">{locale === "en" ? "(required)" : "(wajib)"}</span>
    </>
  );
}
